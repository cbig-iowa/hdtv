function [pdf,val] = genPDF3d(imSize,p,pctg,distType,radius,disp)
...
sy = imSize(2);
sz = imSize(3);
PCTG = floor(pctg*sx*sy*sz);


%if sum(imSize==1)==0  % 3D
	[x,y,z] = meshgrid(linspace(-1,1,sy),linspace(-1,1,sx),linspace(-1,1,sz));
	switch distType
		case 1
